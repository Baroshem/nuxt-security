...
Consider the difference between a safely declared reactive reference (ref) and one that might lead to state pollution:
```ts
<script lang="ts" setup>
// Safe usage: Declared within the setup function
const safeRef = ref<number>(5);

const unSafeFunc = () => {
    // Unsafe usage: Declared inside a function that might be called from different lifecycle hooks
    const unSafeRef = ref<number>(3);
}

onMounted(() => {
    // Calling unsafeFunc here may lead to state pollution
    unSafeFunc();
});
</script>
```
...
<script lang="ts" setup>
// Safe usage: Declared within the setup function
const safeRef = useState<number>('safeRef', () => 5);

const safeFunc = () => {
    // Use with caution: While useState ties state to a request, defining state inside a function like this is not a standard pattern.
    const caution = useState<number>('caution', () => 3);
}
