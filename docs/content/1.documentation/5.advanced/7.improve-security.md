...
1. A script is blocked
  ...
  Our recommendation is to always include external scripts via `useScript` and we provide a detailed section on [how to do this](/headers/csp/#including-external-scripts).
...
2. An image or video is blocked
  ...
  If you need to include external media from third-party sites, please refer to our [documentation here](/headers/csp/#including-external-media).
...
3. A frame is blocked
  ...
  In that case, please follow our instructions on [COEP/COOP compatibility](/headers/crossoriginembedderpolicy/#incompatible-resources).
...
4. Your hosting provider messes with your code
  ...
  Please make sure that you disable all Post-Build Modification services that may be applied by your hosting platform. You can refer to [this section]()
...
6. SSL, TLS, and other HTTPS denials
  ...
  In general, you should not connect to non-HTTPS resources. If you need to disable SSL upgrading in development mode, we provide instructions on [how to do so]().
