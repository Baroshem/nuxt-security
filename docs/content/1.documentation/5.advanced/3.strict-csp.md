...
```ts
useScript('https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js', { trigger: 'idle' })
```
...
Note: you will need to set the `{ trigger: 'idle' }` option if you want to ensure compatibility with SSG mode.
...
We recommend that you insert your external resources with `useScript` and its `{ trigger: 'idle' }` option.

Alternatively you can use `useHead` and its `{ mode: 'client' }` option.

The `{ trigger: 'idle' }` or `{ mode: 'client' }` options will disallow loading your script on the server-side, and will only load it on the client side.

This is necessary to ensure that your script is loaded under `'strict-dynamic'`, and therefore executes in a CSP-compatible way.
...
- Do you need to execute the script on the server-side ? If this is the case, it is likely that you should have bundled your server-side script by way of a NPM module rather than loading it from a URL.
...
Your script can then run isomorphically on the server and on the client by disabling the `trigger` or `mode` options.
...
useScript({
  src: 'https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js',
  crossorigin: 'anonymous',
}) // Do not use trigger: 'idle' option, so that it will also execute on the server
```

With `useHead` you can write:
```ts
useHead({
  script: [
    {
...
}) // Do not use mode: 'client' option, so that it will also execute on the server
```

With `nuxt.config.ts` you can write:
```ts
defineNuxtConfig({
  app: {
    head: {
...
Within HTML templates you can write:
```html
<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/js/bootstrap.bundle.min.js" crossorigin="anonymous">
```

::alert{type="success"}
In SSR mode, you will always be able to load your external scripts under Strict CSP.
::
...
The universal solution is then to select client-only mode by including your script with `useScript` and the `{ trigger: 'idle' }` option.

::alert{type="success"}
In SSG mode, `useScript` with `trigger: 'idle'` will always allow you to load your external scripts under Strict CSP.
::

::alert{type="warning"}
If you have a specific requirement that prevents you from using client-only mode, please continue reading below.
::
